...
export const memberTrashedListService = {
    ...
    getTrashedMembers: async function(): Promise<CardRow[]> {
        window.electron.ipcRenderer.sendMessage(CHANNEL_REQUEST, Cards.selectAllSoftDeleted.name);    
    ...
    setRecorverMemberByFcno: async function(fcno:string): Promise<number> {
        window.electron.ipcRenderer.sendMessage(CHANNEL_REQUEST, Cards.recoveryByFcno.name, fcno);    
    ...
    deleteCompletelyByFcno: async function(fcno:string): Promise<number> {
        window.electron.ipcRenderer.sendMessage(CHANNEL_REQUEST, Cards.deletePhisycalByFcno.name, fcno);    
        const c_val = await window.electron.ipcRenderer.asyncOnce<number>(CHANNEL_REPLY);
        window.electron.ipcRenderer.sendMessage(CHANNEL_REQUEST, Histories.deleteHistoriesByFcno.name, fcno);    
        const h_val = await window.electron.ipcRenderer.asyncOnce<number>(CHANNEL_REPLY);
        return c_val+h_val;
    },
    /** IDMが紐づいたメンバーを取得する */
