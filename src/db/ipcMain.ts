...
export function ipcMainSqliteBridge() {
    const channel = "asynchronous-sql-command";
    const replyChannel = "asynchronous-sql-reply"
    ipcMain.on(channel, async(event:Electron.IpcMainEvent, command:string, ...args:any[])=>{
        ...
        if(command == Cards.cards_updatePersonalDataByFcno.name){
            const fcno:string = args[0];
        ...
        if(command == Histories.hist_insert.name){
            const fcno:string = args[0];
        ...
        if(command == Histories.hist_selectAll.name){
            const rows:HistoriesRow[] = await Histories.hist_selectAll(db);
            event.reply(replyChannel, rows);
            return;
        }
        if(command == Histories.hist_selectByDate.name){
            const date:Date = args[0];
            const rows:HistoriesRow[] = await Histories.hist_selectByDate(db, date);
            console.log(rows);
            event.reply(replyChannel, rows);
            return;
        }// hist_selectRowByFcnoDate
        if(command == Histories.hist_selectRowByFcnoDate.name){
            const fcno:string = args[0]
        ...
        if(command == Histories.hist_selectInRoomByFcnoDate.name){
            const fcno:string = args[0];
            const date:Date = args[1];
        ...
        if(command == Histories.hist_setInRoomByFcnoIdm.name){
            const fcno:string = args[0];
            const idm:string = args[1];
        ...
        if(command == Histories.hist_updateYesterdayToOutroom.name){
